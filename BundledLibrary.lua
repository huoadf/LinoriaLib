...
local proxyObjectsList = {}
MatchaInputManager.Start(proxyObjectsList)

local ScreenGui = MatchaUIProxy.new('ScreenGui');
ProtectGui(ScreenGui);

ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Global;
-- ScreenGui.Parent = CoreGui; (Matcha API is external, so no CoreGui parenting)
...
function Library:Create(Class, Properties)
    ...
    if type(Class) == 'string' then
        _Instance = MatchaUIProxy.new(Class);
        table.insert(proxyObjectsList, _Instance)
    end;
