-- leaked by neuro.net (discord.gg/chudvision) :3
...
getgenv().ForceHit = getgenv().ForceHit or {
    StrafeMode = "Orbit",
    LookAtTarget = false,
...
getgenv().repo = 'https://raw.githubusercontent.com/huoadf/LinoriaLib/main/'
getgenv().libraryFile = 'Library.lua'
...
local Toggles = getgenv().Toggles or (getgenv().Library and getgenv().Library.Toggles) or {}
local Options = getgenv().Options or (getgenv().Library and getgenv().Library.Options) or {}
...
getgenv().AutofarmV2Gb = getgenv().Tabs.Autofarm:AddLeftGroupbox('1v1 Autofarm V2 (Always Win)')
getgenv().AutofarmV2CustomGb = getgenv().Tabs.Autofarm:AddRightGroupbox('V2 Combat & Protection')

getgenv().AutofarmV2Gb:AddToggle('AutofarmV2Enabled', {
    Text = 'Enable Autofarm V2',
    Default = false,
})
getgenv().AutofarmV2Gb:AddDropdown('AutofarmV2QueueMode', {
    Text = 'Queue Mode',
    Values = { 'Auto Snipe Anyone', 'Claim Empty Pad', 'Dual Alt' },
    Default = 'Auto Snipe Anyone',
})
getgenv().AutofarmV2Gb:AddInput('AutofarmV2TargetUser', {
    Default = '',
    Numeric = false,
    Finished = false,
...
getgenv().AutofarmV2Gb:AddDropdown('AutofarmV2WeaponMode', {
    Text = 'Weapon Mode',
    Values = { 'DoubleBarrel', 'Revolver', 'SMG', 'Shotgun', 'Knife' },
    Default = 'DoubleBarrel',
})
getgenv().AutofarmV2Gb:AddToggle('AutofarmV2AntiAFK', {
    Text = 'Anti AFK',
    Default = true,
})
getgenv().AutofarmV2StatsLabel = getgenv().AutofarmV2Gb:AddLabel('Stats: 0 Wins | 0 Kills')

getgenv().AutofarmV2CustomGb:AddToggle('AutofarmV2DesyncProtection', {
    Text = 'Anti-Hit Desync (Invulnerable)',
    Default = true,
    Tooltip = 'Desyncs position during 1v1 so enemy bullets cannot register',
})
getgenv().AutofarmV2CustomGb:AddToggle('AutofarmV2InstantBurst', {
    Text = 'Instant Zero-Ping Headshot',
    Default = true,
    Tooltip = 'Fires multi-pellet headshots instant frame forcefield drops',
})
getgenv().AutofarmV2CustomGb:AddToggle('AutofarmV2InstantStomp', {
    Text = 'Instant Stomp on Down',
    Default = true,
    Tooltip = 'Instantly stomps downed opponent to end match immediately',
})
getgenv().AutofarmV2CustomGb:AddDropdown('AutofarmV2PositionMode', {
    Text = 'Combat Position',
    Values = { 'Behind Target', 'Above Target', 'Sky Platform', 'Orbit Target' },
    Default = 'Behind Target',
})
getgenv().AutofarmV2CustomGb:AddSlider('AutofarmV2SkyHeight', {
    Text = 'Skybox Height',
    Default = 350,
    Min = 50,
    Max = 3000,
...
getgenv().AutofarmV2CustomGb:AddSlider('AutofarmV2ForceFieldDelay', {
    Text = 'ForceField Drop Delay',
    Default = 0.25,
    Min = 0,
    Max = 2,
    Rounding = 2,
})
getgenv().AutofarmV2CustomGb:AddToggle('AutofarmV2AutoFood', {
    Text = 'Auto Food on Low Health',
    Default = true,
})
getgenv().AutofarmV2CustomGb:AddToggle('AutofarmV2AutoReload', {
    Text = 'Auto Reload Weapon',
    Default = true,
})
...
-- =========================================================
-- AUTOFARM V2 (ALWAYS WIN / 1V1 SOLO & ALT ENGINE)
-- =========================================================
do
    local Players = game:GetService("Players")
    local RunService = game:GetService("RunService")
    ...
    getgenv().AutofarmV2State = getgenv().AutofarmV2State or {
        InBox = false,
        Padded = false,
        EnteredAt = 0,
        BasePlate = nil,
    ...
    local function v2Option(name, fallback)
        local o = Options[name]
        if o and o.Value ~= nil then
            return o.Value
        end
    ...
    local function v2UpdateStats()
        if getgenv().AutofarmV2StatsLabel and getgenv().AutofarmV2StatsLabel.SetText then
            local state = getgenv().AutofarmV2State
            getgenv().AutofarmV2StatsLabel:SetText(string.format("Stats: %d Wins | %d Kills", state.Wins, state.Kills))
        end
    end

    local function v2GetChar()
        local char = LocalPlayer.Character
        if not char then return nil end
        local hum = char:FindFirstChildOfClass("Humanoid")
        local root = char:FindFirstChild("HumanoidRootPart")
    ...
    local function v2Teleport(cf)
        local _, _, root = v2GetChar()
        if root then
            pcall(function() root:SetNetworkOwner(LocalPlayer) end)
            root.CFrame = cf
    ...
    local function v2InBoxCheck()
        local folder = workspace:FindFirstChild("Players")
        local inbox = folder and folder:FindFirstChild("InBox")
        return inbox and inbox:FindFirstChild(LocalPlayer.Name) ~= nil or false
    end

    local function v2GetOpponent()
        local targetName = tostring(v2Option('AutofarmV2TargetUser', ''))
        if targetName ~= '' then
            local specific = Players:FindFirstChild(targetName)
            if specific and specific ~= LocalPlayer and specific.Character then
                return specific
            end
        end
        ...
        -- Fallback: check closest player inside any 1v1 arena
        local _, _, myRoot = v2GetChar()
        if myRoot then
    ...
    local function v2WeaponNames(mode)
        if mode == "DoubleBarrel" then
            return { "[Double-Barrel]", "[DoubleBarrel]" }
        elseif mode == "Revolver" then
            return { "[Revolver]" }
        ...
        elseif mode == "Shotgun" then
            return { "[Shotgun]", "[TacticalShotgun]" }
        end
        return { "[Knife]" }
    end

    local function v2EquipWeapon(mode)
        local char, hum = v2GetChar()
        if not char or not hum then return nil end

        for _, name in ipairs(v2WeaponNames(mode)) do
            local equipped = char:FindFirstChild(name)
            if equipped and equipped:IsA("Tool") then
                return equipped
            end
    ...
    local function v2ExpandKnifeHitbox()
        local knife = v2EquipWeapon("Knife")
        local handle = knife and knife:FindFirstChild("Handle")
        local hitbox = handle and (handle:FindFirstChild("HITBOX_PART") or handle)
        if hitbox and hitbox:IsA("BasePart") then
    ...
    local function v2GetPads(fp)
        local j1 = fp and fp:FindFirstChild("JoinFight1")
        local j2 = fp and fp:FindFirstChild("JoinFight2")
        if not j1 or not j2 then return nil, nil end
        return j1:FindFirstChild("Region") or j1, j2:FindFirstChild("Region") or j2
    end

    local function v2FindTargetPad()
        local ignored = workspace:FindFirstChild("Ignored")
        local boxes = ignored and ignored:FindFirstChild("Boxes")
        if not boxes then return nil end

        local queueMode = v2Option('AutofarmV2QueueMode', 'Auto Snipe Anyone')
        local targetUser = tostring(v2Option('AutofarmV2TargetUser', ''))

        -- Mode 1: Target specific user if provided
        if targetUser ~= '' then
            ...
            if targetRoot then
                for _, fp in ipairs(boxes:GetChildren()) do
                    if fp.Name == "FightPlace" then
                        local p1, p2 = v2GetPads(fp)
                        if p1 and (targetRoot.Position - p1.Position).Magnitude < 10 and p2 then
                            return p2
                        elseif p2 and (targetRoot.Position - p2.Position).Magnitude < 10 and p1 then
                            return p1
                        end
                    end
                end
        ...
        -- Mode 2: Auto Snipe Anyone (find pad with 1 waiting player)
        if queueMode == 'Auto Snipe Anyone' then
            for _, fp in ipairs(boxes:GetChildren()) do
                if fp.Name == "FightPlace" then
                    local p1Val = fp:FindFirstChild("Player1")
                    local p2Val = fp:FindFirstChild("Player2")
                    local pad1, pad2 = v2GetPads(fp)
                    if p1Val and p2Val then
                        local hasP1 = (p1Val.Value ~= "" and p1Val.Value ~= nil)
                        local hasP2 = (p2Val.Value ~= "" and p2Val.Value ~= nil)
                        if hasP1 and not hasP2 and pad2 then
                            return pad2
                        elseif hasP2 and not hasP1 and pad1 then
                            return pad1
                        end
                    end
                end
        ...
        -- Mode 3: Claim Empty Pad
        for _, fp in ipairs(boxes:GetChildren()) do
            if fp.Name == "FightPlace" then
                local p1Val = fp:FindFirstChild("Player1")
                local p2Val = fp:FindFirstChild("Player2")
                local pad1, _ = v2GetPads(fp)
                if p1Val and p2Val and pad1 then
                    local hasP1 = (p1Val.Value ~= "" and p1Val.Value ~= nil)
                    local hasP2 = (p2Val.Value ~= "" and p2Val.Value ~= nil)
    ...
    local function v2CreateBasePlate()
        local state = getgenv().AutofarmV2State
        local height = tonumber(v2Option('AutofarmV2SkyHeight', 350)) or 350
        if state.BasePlate and state.BasePlate.Parent then
            state.BasePlate.CFrame = CFrame.new(0, height, 0)
            return state.BasePlate
        end
    ...
    local function v2RemoveBasePlate()
        local state = getgenv().AutofarmV2State
        if state.BasePlate then
            pcall(function() state.BasePlate:Destroy() end)
            state.BasePlate = nil
    ...
    local function v2BuildShootArgs(origin, targetPart)
        local pos = targetPart.Position
        local shootArgs = { {}, {}, origin, origin, workspace:GetServerTimeNow() }
        for i = 1, 5 do
            shootArgs[1][i] = { Normal = pos, Instance = targetPart, Position = pos }
            shootArgs[2][i] = { thePart = targetPart, theOffset = Vector3.zero }
        end
        return shootArgs
    end

    local function v2PerformInstantKill(target)
        local targetChar = target and target.Character
        if not targetChar then return end
        local head = targetChar:FindFirstChild("Head") or targetChar:FindFirstChild("HumanoidRootPart")
        local _, _, myRoot = v2GetChar()
        if not (head and myRoot and MainEvent) then return end

        local mode = v2Option('AutofarmV2WeaponMode', 'DoubleBarrel')
        if mode == "Knife" then
            v2ExpandKnifeHitbox()
            local knife = v2EquipWeapon("Knife")
            if knife then
            ...
            v2EquipWeapon(mode)
            local args = v2BuildShootArgs(myRoot.Position, head)
            MainEvent:FireServer("Shoot", args)
        end
        getgenv().AutofarmV2State.LastFireTick = tick()
    end

    local function v2PerformStomp(target)
        local targetChar = target and target.Character
        local torso = targetChar and (targetChar:FindFirstChild("UpperTorso") or targetChar:FindFirstChild("Torso") or targetChar:FindFirstChild("HumanoidRootPart"))
        local _, _, myRoot = v2GetChar()
        if not (torso and myRoot and MainEvent) then return end

        getgenv().AutofarmV2State.IsStomping = true
        v2Teleport(CFrame.new(torso.Position + Vector3.new(0, 2.8, 0)))
        MainEvent:FireServer("Stomp")
        task.wait(0.08)
        getgenv().AutofarmV2State.IsStomping = false
    end

    local function v2HandleAutoFood()
        if not v2Toggle('AutofarmV2AutoFood') then return end
        local _, hum = v2GetChar()
        if not hum or hum.Health <= 0 then return end
        local maxH = hum.MaxHealth > 0 and hum.MaxHealth or 100
        if (hum.Health / maxH) > 0.65 then return end

        local food = nil
        local char = LocalPlayer.Character
        if char then
            for _, item in ipairs(char:GetChildren()) do
                if item:IsA("Tool") and (item.Name:find("Pizza") or item.Name:find("Taco") or item.Name:find("Chicken") or item.Name:find("Burger")) then
                    food = item; break
                end
            end
        end
        if not food and LocalPlayer:FindFirstChild("Backpack") then
            for _, item in ipairs(LocalPlayer.Backpack:GetChildren()) do
                if item:IsA("Tool") and (item.Name:find("Pizza") or item.Name:find("Taco") or item.Name:find("Chicken") or item.Name:find("Burger")) then
                    hum:EquipTool(item); food = item; break
                end
            end
        end
        if food then pcall(function() food:Activate() end) end
    end

    local function v2HandleAutoReload()
        if not v2Toggle('AutofarmV2AutoReload') then return end
        local state = getgenv().AutofarmV2State
        if tick() - state.LastReloadTick < 1 then return end
        local char = LocalPlayer.Character
        if not char then return end
        ...
        if a and a:IsA("IntValue") and a.Value == 0 then
            ...
            state.LastReloadTick = tick()
        end
    end

    -- Anti AFK for V2
    LocalPlayer.Idled:Connect(function()
        if v2Toggle('AutofarmV2AntiAFK') or v2Toggle('AutofarmV2Enabled') then
            VirtualUser:Button2Down(Vector2.new(0, 0), workspace.CurrentCamera.CFrame)
            task.wait(0.1)
            VirtualUser:Button2Up(Vector2.new(0, 0), workspace.CurrentCamera.CFrame)
        end
    ...
    -- Master V2 Execution Thread
    task.spawn(function()
        local orbitAngle = 0
        while task.wait(0.04) do
            if not v2Toggle('AutofarmV2Enabled') then
                getgenv().AutofarmV2State.InBox = false
                getgenv().AutofarmV2State.Padded = false
                getgenv().AutofarmV2State.EnteredAt = 0
                v2RemoveBasePlate()
                continue
            end

            local char, hum, root = v2GetChar()
            if not char or not hum or not root or hum.Health <= 0 then continue end
            ...
            if v2InBoxCheck() then
                ...
                local opponent = v2GetOpponent()
                local oppChar = opponent and opponent.Character
                local oppHum = oppChar and oppChar:FindFirstChildOfClass("Humanoid")
                local oppRoot = oppChar and oppChar:FindFirstChild("HumanoidRootPart")
                local oppBody = oppChar and oppChar:FindFirstChild("BodyEffects")
                local oppKO = oppBody and oppBody:FindFirstChild("K.O") and oppBody["K.O"].Value
                local oppDead = (oppHum and oppHum.Health <= 0) or (oppBody and oppBody:FindFirstChild("SDeath") and oppBody["SDeath"].Value)

                -- Check if opponent is downed (K.O) -> Instant Stomp to Win
                if oppKO and not oppDead then
                    if v2Toggle('AutofarmV2InstantStomp') and not state.IsStomping then
                        v2PerformStomp(opponent)
                    end
                elseif oppDead then
                    -- Win confirmed!
                    if state.CurrentOpponent == opponent then
                        state.Wins = state.Wins + 1
                        state.Kills = state.Kills + 1
                        state.CurrentOpponent = nil
                        v2UpdateStats()
                    end
                elseif oppChar and oppRoot and oppHum and oppHum.Health > 0 then
                    state.CurrentOpponent = opponent
                    local posMode = v2Option('AutofarmV2PositionMode', 'Behind Target')

                    -- Positioning Engine
                    if posMode == 'Behind Target' then
                        local behindPos = oppRoot.Position - (oppRoot.CFrame.LookVector * 3.5) + Vector3.new(0, 1.5, 0)
                        v2Teleport(CFrame.lookAt(behindPos, oppRoot.Position))
                    elseif posMode == 'Above Target' then
                        local abovePos = oppRoot.Position + Vector3.new(0, 6.5, 0)
                        v2Teleport(CFrame.lookAt(abovePos, oppRoot.Position))
                    elseif posMode == 'Orbit Target' then
                        orbitAngle = orbitAngle + 0.25
                        local orbitPos = oppRoot.Position + Vector3.new(math.cos(orbitAngle) * 5, 2, math.sin(orbitAngle) * 5)
                        v2Teleport(CFrame.lookAt(orbitPos, oppRoot.Position))
                    elseif posMode == 'Sky Platform' then
                        local plate = v2CreateBasePlate()
                        if plate then
                            v2Teleport(plate.CFrame + Vector3.new(0, 3, 0))
                        end
                    end

                    -- Desync Protection in match
                    if v2Toggle('AutofarmV2DesyncProtection') and getgenv().desync then
                        getgenv().desync.enabled = true
                    end

                    -- Combat Execution
                    local ff = oppChar:FindFirstChildOfClass("ForceField")
                    local ffDelay = tonumber(v2Option('AutofarmV2ForceFieldDelay', 0.25)) or 0.25
                    local canAttack = (not ff) and (tick() - state.EnteredAt >= ffDelay)

                    if canAttack then
                        v2PerformInstantKill(opponent)
                    end
                end
            else
                -- Outside InBox (In Lobby / Finding Pads)
                if state.InBox then
                    state.InBox = false
                    state.Padded = false
                    state.EnteredAt = 0
                ...
                if not state.Padded then
                    local targetPad = v2FindTargetPad()
                    if targetPad then
                        state.Padded = true
                        task.spawn(function()
                            while v2Toggle('AutofarmV2Enabled') and not v2InBoxCheck() do
                                local pad = v2FindTargetPad()
                                if pad then
                                    v2Teleport(pad.CFrame + Vector3.new(0, 3, 0))
                                end
                                task.wait(0.12)
                            end
                            state.Padded = false
...
local KillAura = getgenv().Tabs.Main:AddRightGroupbox("Strafe")
